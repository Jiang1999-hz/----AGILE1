        ...
        <article class="panel">
          ...
          <div class="course-program-list">
            ${ctx.courses().map((item) => `
              <button class="course-program-card ${course?.id === item.id ? "active" : ""}" data-course-id="${item.id}" type="button">
                <div class="panel-head">
                  <div>
                    <p class="eyebrow">${item.subject}</p>
                    <h4>${item.title}</h4>
                  </div>
                  <span class="tag">${item.completedLessons}/${item.totalLessons}</span>
                </div>
                <div class="course-progress-bar"><i style="width:${ctx.courseProgress(item)}%"></i></div>
                <div class="course-meta-list">
                  <div class="section-row"><strong>进度</strong><span>${ctx.courseProgress(item)}%</span></div>
                  <div class="section-row"><strong>下节课</strong><span>${item.nextLesson}</span></div>
                </div>
              </button>
            `).join("")}
          </div>
        ...
        <article class="panel">
          <div class="panel-head">
            <div>
              ...
              <h3>${course?.title || "课程日历"}</h3>
            </div>
          </div>
          <div class="calendar-board">
            ${courseLessons.map((item) => `
              <button class="lesson-card ${currentLesson?.id === item.id ? "active" : ""}" data-lesson-id="${item.id}" type="button">
        ...
        <article class="panel">
          ${currentLesson ? `
            <div class="panel-head">
              ...
              <span class="tag">${currentLesson.date} ${currentLesson.time}</span>
            </div>
    ...
    return `
      <section class="student-dashboard">
        ...
        <article class="panel">
          ...
          <h3>最近进度</h3>
          <div class="sparkline">${(records.progress || []).map((value) => `<i style="height:${Math.max(18, value * 0.8)}px"></i>`).join("")}</div>
        </article>
        <article class="panel">
          <p class="eyebrow">Parent Summary</p>
          <h3>家长摘要</h3>
          <p class="profile-meta">${student?.parentNote || ""}</p>
        </article>
      </section>
      <section class="grid-2" style="margin-top:18px;">
          ...
          <div class="todo-list">
            ${(records.homework || []).map((item) => `
              <div class="todo-item">
                ...
                <p>${item.status} / ${item.score ?? "待评分"} / ${item.teacherNote || "暂无点评"}</p>
              </div>
            `).join("")}
          </div>
        </article>
        <article class="panel">
          ...
          <h3>做题记录</h3>
          <div class="todo-list">
            ${(records.quizHistory || []).slice().reverse().map((item) => `
              <div class="todo-item">
